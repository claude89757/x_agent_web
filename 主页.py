...
if st.session_state.get('access_code') and st.session_state.get('access_code') in os.environ['ACCESS_CODE_LIST'].split(','):
    # session中有缓存
    st.query_params.access_code = st.session_state.access_code
    access_granted = True
elif st.query_params.get('access_code') and st.query_params.get('access_code') in os.environ['ACCESS_CODE_LIST'].split(','):
    # URL中有缓存
    ...
    if st.button(submit_button_label, type="primary"):
        if code in os.environ['ACCESS_CODE_LIST'].split(','):
            access_granted = True
...
if access_granted:
    ...
    st.page_link("pages/1_小红书_智能助手.py", label="小红书智能助手", icon="1️⃣", use_container_width=True)
    st.markdown("-----")
